...
func LogAndSendError(message string, err error, severity operations.Severity, task *components.Task) error {
	state.ErrorLogger.Error(message, "error", err)
	...
	if task != nil {
		...
		_, err := state.State.SdkClient.Agents.SubmitErrorAgent(context.Background(), state.State.AgentID, agentError)
		if err != nil {
			state.Logger.Error("Error sending error to server", "error", err)
		}
	}
