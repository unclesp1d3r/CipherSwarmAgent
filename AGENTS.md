...
- `lib/`: Core agent logic, decomposed into focused sub-packages:
  ...
  - `monitor/`: Background system performance monitoring. Samples CPU, memory, swap, system load, cumulative disk/network I/O, and optional per-process metrics via `gopsutil/v4` on a configurable interval and reports them through the structured logger. The `Source` interface abstracts gopsutil for testability (`gopsutilSource` is the production impl); `Monitor.Collect` degrades gracefully (load-average gaps, disk/net counter gaps, and process errors are soft, while core CPU/memory/swap failures return partial metrics + error). Disk I/O aggregation filters partition entries to avoid double-counting on Linux; repeated disk/net soft failures log only on availability transitions. Per-process sampling targets the running hashcat PID (published to `agentstate` from `hashcat.Session.Start`/`Cleanup` via `SetHashcatPID`/`ClearHashcatPID`), falling back to the agent process when idle. No server endpoint exists in the v1 Agent API, so metrics are local-only; GPU temp/utilization already flow via `DeviceStatus` task updates.
