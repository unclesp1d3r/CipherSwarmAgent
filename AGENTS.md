...
- `lib/`: Core agent logic, decomposed into focused sub-packages:
  ...
  - `monitor/`: Background system performance monitoring. Samples CPU, memory, swap, system load, and optional per-process metrics via `gopsutil/v4` on a configurable interval and reports them through the structured logger. The `Source` interface abstracts gopsutil for testability (`gopsutilSource` is the production impl); `Monitor.Collect` degrades gracefully (load-average gaps and process errors are soft, core-counter failures return partial metrics + error). No server endpoint exists in the v1 Agent API, so metrics are local-only; GPU temp/utilization already flow via `DeviceStatus` task updates.
